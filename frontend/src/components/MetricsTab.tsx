...
function computeMetrics(result: AnalysisResult) {
  ...
  const topByDegree = [...degreeMap.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([path, deg]) => ({ name: path.split(/[\\/]/).pop() ?? path, fullPath: path, links: deg }));
  ...
  return { topByDegree, topByInDegree, histData, components, totalNodes, totalLinks, ratio };
}
...
function TopList({ items }: { items: Array<{ name: string; fullPath: string; links: number }> }) {
  ...
  return (
    <div className="rp-top-list">
      {items.map(({ name, fullPath, links }, i) => (
        <div key={fullPath} className="rp-top-row">
          <span className="rp-top-rank">{i + 1}</span>
          <span className="rp-top-name" title={fullPath}>{name}</span>
...
export function MetricsTab({ result, derived }: Props) {
  if (!result || !derived) {
    return (
  ...
  const {
    topByDegree, topByInDegree, histData,
