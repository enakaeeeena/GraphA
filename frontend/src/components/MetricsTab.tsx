...
function computeMetrics(result: AnalysisResult) {
  ...
  for (const file of result.files) {
    for (const dep of file.dependencies ?? []) {
      if (dep.import_type?.toLowerCase() === 'dynamic' || dep.import_type?.toLowerCase() === 'dynamic_import') {
        filesWithDynamic.add(file.file_path);
      }
    }
  }
  const dynamicCount = filesWithDynamic.size;
  ...
  return { topByDegree, topByInDegree, topByCentrality, histData, components, totalNodes, totalLinks, ratio, dynamicCount, filesWithDynamic };
}
...
export function MetricsTab({ result, derived, selectedPath, onSelectFile }: Props) {
  ...
  const {
    ...
    components, totalNodes, totalLinks, ratio, dynamicCount,
      ...
      <Accordion title="Проблемы архитектуры">
        <div className="rp-problems">
          ...
          <div className="rp-problem-card">
            <span className="rp-problem-dot rp-problem-dot--yellow" />
            <div>
              <div className="rp-problem-title">Динамические зависимости</div>
              <div className="rp-problem-sub">
                {dynamicCount ? `найдено в ${dynamicCount} файлах` : 'не найдено'}
              </div>
            </div>
          </div>
          {/* <div className="rp-problem-card">
